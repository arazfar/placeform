...
- `AIAND_API_KEY`: Optional access to earlier AIand films. Their history and cached playback remain available in Film; refresh retrieves provider status, and uncached playback requires this connection. New 3D model films render locally without a provider credential or charge.
...
In Film, the active 3D model is the source of truth. Preview its four-shot sequence, then select **Generate cinematic** to render a silent 24-second, 1080p, 24 fps film on this device. The sequence includes an approach, orbit, detail and elevated reveal, with geometry-aware clearance and facade sightline checks. Original materials and the selected daylight setting are retained. H.264 MP4 is preferred; browsers without H.264 encoding use explicitly labeled VP9 WebM. A browser with WebGL2 and WebCodecs video encoding is required.

Rendering snapshots the selected model before asynchronous work. Changing models or moving to another panel does not change that take; keep the page open until rendering finishes. Cancel stops the current render. Reloaded unfinished takes are marked interrupted, and **Render saved model again** uses their captured GLB and sequence. Videos and source snapshots persist in IndexedDB on this browser. Download the video or export its ZIP containing the GLB, specification, source identity, sequence, thumbnail and completed video. If storage fills after encoding, the completed video remains downloadable for the current session. Earlier AIand films remain in a separate history section.
