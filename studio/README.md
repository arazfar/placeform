...
- `AIAND_API_KEY`: AIand video API. Film automatically checks the connection and shows whether the key owner has accepted Video Service Terms. If needed, use its direct AIand Video console link, accept the terms, then refresh the connection. The app checks the live `/v1/videos/models` catalog for `minimaxai/minimax-h3`, verifies the 768p rate again before submitting, uploads first/last model frames, and requires the displayed Generate action. See `docs/video-research.md` for verified provider inputs and pricing. Unknown submission outcomes require history reconciliation before retrying; the app never automatically repeats a paid POST.
...
In Film, wait for the automatic AIand connection check, prepare model frames, review the live price and prompt, then generate. Review completed video for architectural drift against the retained original model clip.
