...
## Implemented cinematic workflow and pilot — 2026-09-09

Film now takes an explicit `filmReference(spec, selected, label)` from Studio, using the same `conceptImage` resolver as the concept view. The default is a 10-second first-frame take, slow small-amplitude push-in, unchanged reference lighting and geometry, quiet outdoor ambience, and no music. The lateral alternative uses a small truck-right move. Model frames remain an explicit advanced workflow. Non-16:9 reference images are contained inside a 1920×1080 PNG, with black borders rather than cropping or distortion.

Prompt structure follows MiniMax's [official image-to-video writing guide](https://huggingface.co/MiniMaxAI/MiniMax-H3/blob/main/docs/VIDEO_PROMPT_WRITING_GUIDE_base_en.md): frame alignment, `integrated_multimodal_description`, `overall_soundscape`, and `non_diegetic_music`. No unsupported provider resolution, seed, quality, or negative-prompt fields are sent.

The server adapter validates catalog, uploads, jobs, references, and media responses; retries safe reads only; retains provider error codes; distinguishes preflight/upload failures from uncertain job submissions; and preserves Range headers for playback. References are saved as immutable browser media before a synchronously persisted submission intent. Provider receipts preserve local settings. Returning to Film reads durable receipts, immediately resumes active jobs, and reconciles ambiguous requests without assigning the same provider job to multiple intents. Completed media is cached in IndexedDB while Film is open; failures remain visible and downloads remain available.

Paid pilot: `video_d9fb6a82f3d349f3958ff33431ea7310`, Terrace Commons reference `datacenter_2.png`, 10 seconds, slow push-in, AIand MiniMax H3. Submitted through the existing credentialed Site API using the new shared prompt; AIand accepted and completed the render for USD 0.80. The downloaded artifact is H.264 1344×768 at 24 fps, with AAC audio, 10.125 seconds, 6,881,597 bytes. Requested aspect ratio was 16:9; the engine's measured encoded dimensions are reported here without relabeling them as 1080p or 2K.

Sampled frames at two-second intervals preserve the stepped terrace arrangement, sandstone material identity, glazing, vegetation and bay background, with gradual viewpoint movement and no obvious cut or large structural transformation. This is a sampled visual review, not a guarantee against fine-detail drift. The audio stream decodes successfully and is very quiet (mean −51.9 dB, peak −38.3 dB); subjective listening was unavailable in the agent environment, so speech/music absence is not certified. Local pilot artifacts are retained in ignored `outputs/film-pilot/` for review.
